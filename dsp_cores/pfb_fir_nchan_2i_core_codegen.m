function pfb_top_filepath = pfb_fir_nchan_2i_core_codegen(destination_filepath, npoints)
    ...
    sprintf("entity pfb_fir_%dc_2i_core_ip_struct is",npoints)
    "  port ("
    ...
    "    ce_1 : in std_logic"
    "  );"
    sprintf("end pfb_fir_%dc_2i_core_ip_struct;",npoints)
    ""
    sprintf("architecture structural of pfb_fir_%d_2i_core_ip_struct is",npoints)
    "begin"
    sprintf("  pfb_fir_%dc_2i_core_ip_inst : entity pfb_fir_%dc_2i_core",npoints,npoints)
    "  port map ("
    "    pol_in => pol_in,"
    "    pol_out => pol_out,"
    "    sync     => sync,      "
    ...
    pfb_top_filepath = [destination_filepath sprintf('/pfb_fir_%dc_2i_core_ip_struct.vhd',npoints)];
